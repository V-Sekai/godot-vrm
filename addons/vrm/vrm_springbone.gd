...
func setup_recursive(skeleton: Skeleton, id: int, center_tr) -> void:
	if skeleton.get_bone_children(id).empty():
		...
		verlets.append(VRMSpringBoneLogic.new(skeleton, id, center_tr, child_position, skeleton.get_bone_global_pose_without_override(id, true)))
	else:
		...
		verlets.append(VRMSpringBoneLogic.new(skeleton, id, center_tr, pos, skeleton.get_bone_global_pose_without_override(id, true)))
	for child in skeleton.get_bone_children(id):
		setup_recursive(skeleton, child, center_tr)
	return

# Called when the node enters the scene tree for the first time.
func _ready(skel: Skeleton, colliders_ref: Array):
	setup(skel)
...
class VRMSpringBoneLogic:
	...
	func get_transform(skel: Skeleton) -> Transform:
		return skel.global_transform * skel.get_bone_global_pose_without_override(bone_idx)
	func get_rotation(skel: Skeleton) -> Quat:
		return get_transform(skel).basis.get_rotation_quat()
	
	func get_local_transform(skel: Skeleton) -> Transform:
		return skel.get_bone_global_pose_without_override(bone_idx)
	func get_local_rotation(skel: Skeleton) -> Quat:
		return get_local_transform(skel).basis.get_rotation_quat()
	
	func reset(skel: Skeleton, transform: Transform) -> void:
	...
	func update(skel: Skeleton, center, stiffness_force: float, drag_force: float, external: Vector3, colliders: Array) -> void:
		...
		var next_tail: Vector3 = tmp_current_tail + (tmp_current_tail - tmp_prev_tail) * (1.0 - drag_force) + get_rotation(skel).xform(bone_axis) * stiffness_force + external
		
		# Limiting bone length
		next_tail = get_transform(skel).origin + (next_tail - get_transform(skel).origin).normalized() * length
		...
		# Apply rotation
		var ft = VRMTopLevel.VRMUtil.from_to_rotation(get_rotation(skel).xform(bone_axis), next_tail - get_transform(skel).origin)
		if ft != null:
			ft = skel.global_transform.basis.get_rotation_quat().inverse() * ft
			var qt: Quat = ft * get_rotation(skel)
			var tr: Transform = get_local_transform(skel)
			tr.basis = Basis(qt.normalized())
			skel.set_bone_global_pose_override(bone_idx, tr, 1.0)
		
		return
	
	func collision(skel: Skeleton, colliders: Array, _next_tail: Vector3) -> Vector3:
		...
		for collider in colliders:
			...
			if (diff.x * diff.x + diff.y * diff.y + diff.z * diff.z) <= r * r:
				...
				# Limiting bone length
				out = get_transform(skel).origin + (pos_from_collider - get_transform(skel).origin).normalized() * length
		return out
